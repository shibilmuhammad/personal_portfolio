...
const ExperienceCard = ({ index, company, position, desc, institute, degree, duration }) => {
  ...
  return (
    <div className={`mb-6 md:mb-8 flex md:justify-between items-center w-full ${index % 2 === 0 ? 'md:flex-row-reverse left-timeline' : 'right-timeline'}`}>
      ...
      <span className="z-20 flex items-center order-1 justify-center w-6 h-6 md:w-9 md:h-9 bg-violet-200 rounded-full ring-4 md:ring-8 ring-white dark:ring-grey-800 dark:bg-violet-900">
        {company && < i className="text-base md:text-xl text-violet-600 dark:text-violet-400 material-symbols-outlined" >work</i>}
        {institute && <i className="text-base md:text-xl text-violet-600 dark:text-violet-400 material-symbols-outlined" >school</i>}
      </span>
      ...
      <motion.div
        ...
        <h3 className="mb-2 font-medium text-lg md:text-xl dark:text-white">{company || institute}</h3>
